...
import com.jhworks.library.crop.callback.OverlayViewChangeListener
import com.jhworks.library.utils.RectUtils
...
class OverlayView(context: Context, attrs: AttributeSet?, defStyleAttr: Int)
    ...
    private fun getCurrentTouchIndex(touchX: Float, touchY: Float): Int {
        ...
        var i = 0
        while (i < 8) {
            val distanceToCorner = Math.sqrt(Math.pow(touchX - mCropGridCorners!![i].toDouble(), 2.0)
        ...
        if (mShowCropGrid) {
            if (mGridPoints == null && !mCropViewRect.isEmpty) {
                ...
                for (i in 0 until mCropGridRowCount) {
                    mGridPoints?.let {
                        it.set(index++, mCropViewRect.left)
                        it.set(index++, mCropViewRect.height() * ((i.toFloat() + 1.0f) / (mCropGridRowCount + 1).toFloat()) + mCropViewRect.top)
                        it.set(index++, mCropViewRect.right)
                        it.set(index++, mCropViewRect.height() * ((i.toFloat() + 1.0f) / (mCropGridRowCount + 1).toFloat()) + mCropViewRect.top)
                    }
                }
                for (i in 0 until mCropGridColumnCount) {
                    mGridPoints?.let {
                        it.set(index++, mCropViewRect.width() * ((i.toFloat() + 1.0f) / (mCropGridColumnCount + 1)) + mCropViewRect.left)
                        it.set(index++, mCropViewRect.top)
                        it.set(index++, mCropViewRect.width() * ((i.toFloat() + 1.0f) / (mCropGridColumnCount + 1)) + mCropViewRect.left)
                        it.set(index++, mCropViewRect.bottom)
                    }
                }
            }
            if (mGridPoints != null) {
